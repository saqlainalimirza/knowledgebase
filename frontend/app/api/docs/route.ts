...
export async function GET() {
  ...
  const docs = {
    ...
    stats: [
      ...
      { m: "GET|PATCH", path: "/api/clients/{slug}/learnings", summary: "Durable learnings distilled from real A/B (nightly): 'variant X beat Y' with n's, delta, confidence. confirmed=passed significance gate, proposed=directional. NOTE the rate is reply-quality (÷ repliers), not per send. PATCH /api/learnings {id, status:'confirmed'|active:false}.", params: "status" },
